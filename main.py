        ...
        try:
            if "status" in event_json:
                if "sub_state" in event_json and event_json["sub_state"] != "":
                    ...
                    print(
                        "amd_event_url: Call event detected --> {0}".format(
                            event_json["status"]
                        )
                    )
            else:
                print(
                    "amd_event_url: An event detected --> {0}".format(
                        event_json["status"]
                    )
                )
...
def answer_url():
    ...
    for arg in request.args.keys():
        event_json[arg] = request.args.get(arg)
    ncco = [
        { "action": "conversation", "name": "conference_{0}".format(event_json["uuid"]),
        # "muted": False,
        # "record": False,
        # "earmuff": True,
        # "playExitSound": True,
    ...
    ncco = [
        {
            ...
            "endpoint": [ { "type": "sip", "uri": event_json["uri"],
               "standard_headers": { "User-to-User": "342342ef34;encoding=hex;purpose=foo;content=bar" },
               "headers": { "story": "gruffalo", "occupation": "developer" } }
             ]
        }
    ]
    print("\n*****\nanswer_url --> Sending NCCO: {0}\n*****\n".format(ncco))
...
def asr():
    ...
    ncco = [
        ...
        },
        {
            ...
            "action": "input",
            "eventUrl": [
                "http://qaservices1.flint.npe:8555/callback?qa_type=asr"
            ]
        }
    ]
    print("\n*****\nanswer_url --> Sending NCCO: {0}\n*****\n".format(ncco))
                    ...
                    ncco = [
                        {
                            ...
                            "voiceName": "Amy",
                            "level": "1",
                            "text": "Thank you for your input {0}. Please wait for stream action.".format(
                                event_json["dtmf"]["digits"]
                            ),
                        },
                        {
                            "action": "stream",
                    ...
                    ncco = [
                        {
                            ...
                            "voiceName": "Amy",
                            "level": "1",
                            "text": "Thank you for your input {0}. Please wait for record and connect action.".format(
                                event_json["dtmf"]["digits"]
                            ),
                        },
                        # {
                        ...
                        {
                            "action": "connect",
                            "endpoint": [{"type": "phone", "number": "441174960064"}],
                            "limit": "180",
                        },
                    ]
                    ...
                    ncco = [
                        {
                            ...
                            "voiceName": "Amy",
                            "level": "1",
                            "text": "Thank you for your input {0}. Please wait for conversation action.".format(
                                event_json["dtmf"]["digits"]
                            ),
                        },
                        {
                    ...
                    ncco = [
                        {
                            ...
                            "voiceName": "Amy",
                            "level": "1",
                            "text": "Thank you for your input {0}. We will now hangup the call.".format(
                                event_json["dtmf"]["digits"]
                            ),
                        },
                        {"action": "hangup"},
...
def ncco_event_url():
    if request.is_json:
        ...
        if "recording_url" in request.get_json():
            ...
            print("\n>>>>>\speech: {0}\n<<<<<\n".format(request.get_json()["speech"]))
